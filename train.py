...
from dataset import MagAudioDataset
...
class MagDataModule(pl.LightningDataModule):
    """
    Loads a single (mag.npy, audio.npy) bundle and splits it
    ...
    def setup(self, stage: str | None = None) -> None:
        full_dataset = MagAudioDataset(self.data_dir)
        n = len(full_dataset)
        ...
        # Deterministic split every call
    ...
    def train_dataloader(self):
        return DataLoader(self.train_set, batch_size=self.bs, shuffle=True,
                          num_workers=self.nw, pin_memory=True)

    def val_dataloader(self):
        return DataLoader(self.val_set,   batch_size=self.bs, shuffle=False,
                          num_workers=self.nw, pin_memory=True)
...
def main() -> None:
    ...
    p.add_argument("--mode",       type=str,  default="convergence",
                   choices=["convergence", "overall"])
    args = p.parse_args()
    ...
    model = DeepAGLA(
        ...
        initial_params = BEST_INITIAL_COMBINATIONS[args.mode],
    )
    dm = MagDataModule(args.data_dir, args.batch_size)
